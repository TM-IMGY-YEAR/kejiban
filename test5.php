...
if (isset($_POST['Login'])) {
	...
	if ($_POST['Login'] == 'ログイン') {
		...
		if (isBlank($err)) {
			$stmt = $db->DbFunction()->prepare("select USER_ID,
					 ...
					 USER_ID=:userID AND USER_PASS=:userPS;");

			//1つ目=stmt内ID,2つ目=入力フォーム,3つ目=文字列ということ
			$stmt->bindValue(":userID", $_POST['userID'], PDO::PARAM_STR);
			$stmt->bindValue(":userPS", $_POST['password'], PDO::PARAM_STR);
	...
	//FC $_SESSION["actionName"]に"index_display"を格納
	}else{
...
<!-- エラー項目赤字で出るようになっている -->
		...
		<form action="" method="POST" name="Form1">
			...
			<p>
				...
				<input type="password" name="password" value="<?php if (!empty($_POST["password"])) {echo htmlspecialchars($_POST["USER_PASS"], ENT_QUOTES);} ?>">
			</p>
			<div>
